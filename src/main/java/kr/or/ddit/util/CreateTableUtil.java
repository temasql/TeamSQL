...
public class CreateTableUtil {
	...
	*/
	private String pkCheckList(List<String> pkCheckList, List<String> pkColNameList, 
			String tableName, StringBuffer query) {
	...
	private List<String> createCommentQuery(List<String> commentList, String account_id,
			String tableName, List<String> commentColNameList){
	...
	*/
	protected Map<String, Object> getQuery(String[][] array) {
...
//		}
		...
		// pk 생성 쿼리
		query.append(pkCheckList(pkCheckList, pkColNameList, tableName, query));
...
//			}
//		}
		commentQueryList = createCommentQuery(commentList, account_id, tableName, commentColNameList);
	 ...
	 * Method 설명 : 테이블을 생성할때 JSON 배열로 받아온 컬럼의 갯수와 형태에 맞게 쿼리를 문자열로 반환해주는 메서드
	 */
	public static Map<String, Object> getUpdateQuery(String[][] array) {
		...
		String dropQuery = "";
		queryMap.put("stNm", account_id + "." + tableName);
		// ALTER TABLE SCHEMA.TABLENAME ADD(
		for (int i = 0; i < array.length; i++) {
			query.append("ALTER TABLE " + account_id + "." + tableName + "\nADD (");
			...
			// 기본값을 입력했을 경우
			if(ctu.colDefaultCheck(dataType,defaultVal)) {
				query.append(" DEFAULT " + defaultVal);
			}
			// primary key를 체크했을때 pk를 체크한 컬럼의 이름과 pk체크 횟수를 리스트 리스트
			if(pkCheck.equals("true")) {
		...
		if (pkCheckList != null && pkCheckList.size() > 0) {
			...
			for (int i = 0; i < pkCheckList.size(); i++) {
				...
				// 반복문 중 pk를 생성할때 작성하는 쿼리 문법이 없으면 true
				if(!pkQuery.toString().contains("ALTER TABLE "+ account_id + "." + tableName +" \nADD CONSTRAINT " + tableName + "_PK PRIMARY KEY\n(\n")) {
					
					// pk생성 쿼리 작성
					pkQuery.append("ALTER TABLE "+ account_id + "." + tableName +" \nADD CONSTRAINT " + tableName + "_PK PRIMARY KEY\n(\n");
			...
			// pk생성 끝
			pkQuery.append(")\n ENABLE");
			queryMap.put("pkQuery", pkQuery.toString());
		}
